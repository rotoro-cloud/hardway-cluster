...
В моем случае `controlplane01` является также сервером CA кластера, поэтому мы будем хранить его там. Т.е. у меня нет необходимости его куда-то копировать
...
### Серверный сертиикат для `kube-apiserver`
